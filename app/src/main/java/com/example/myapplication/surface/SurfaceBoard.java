package com.example.myapplication.surface;
...
import com.example.myapplication.penNew.BitmapStep;
import com.example.myapplication.penNew.SteelPen;
import com.example.myapplication.penNew.WritingState;
import com.example.myapplication.penNew.WritingStep;
...
public class SurfaceBoard extends SurfaceView implements SurfaceHolder.Callback, Runnable {
    ...
    private volatile float scale = 0;  //记录旧的缩放标记，判断是放大还是缩小
    ...
    private void drawToCanvas(){
        try {
            ...
            if (canvas != null) {
                ...
                repaintAll(canvas);
                if (isEraser) {
                    canvas.drawRect(lastX, lastY, lastX + 100, lastY + 100, eraserPaint);
                } else {
                    if (path != null && !isRoam) {
                        if (!isSelect)
